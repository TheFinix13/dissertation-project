...
**Date:** 30 July 2026 (formulas cleaned for readability 6 Aug 2026)  
**Purpose:** Show understanding + a working policy-RL loop + first real-data table  
**Git branch:** `simple-modelling-clean` (Nguyen simple-modelling track)
...
| Piece | Definition |
|---|---|
| State | `s = [ΔP, C, n]`  — price change, cash, share **count** (not vague Hold flag `H ∈ {0,1}`) |
| Actions | `{0: Hold, 1: Buy one, 2: Sell one}` with feasibility mask |
| Wealth | `W = C + n · P` |
| Reward | `r = W_next − W` |
| Episode (Phase 1 now) | **one calendar month of daily closes** (`T ≈ 18–23`) — honest interim; minute `T = 390` is next |
| Learning | PPO on policy `π_θ`; value loss on critic `V_φ` |
...
**Honest finding (doer, not recorder):** deterministic PPO matched B1b on **26/26** test months. With this state and `ΔW` reward, the learned policy *is* fully invested buy-and-hold. We only saw that because we added the fair buy-max baseline (one-share BAH looks “beaten” for the wrong reason).
...
Next doer extras if you want them: from-scratch REINFORCE (no SB3), reward `= ΔW − λ · D` (drawdown), fee sensitivity, one real minute-bar day.
...
3. Given PPO collapsed to buy-max under pure `ΔW`, should Iteration-2 start with a **drawdown / turnover penalty**, or with a **richer state** first?
