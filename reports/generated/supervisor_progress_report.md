...
The four-agent comparison run across the entire broad test universe (70 stocks) at the Phase-1 budget (3 seeds × 10 000 PPO timesteps per cell):
...
The broad-universe evidence from Section 5.5 / Appendix B of the academic dissertation can also be presented in a companion document written for a non-quantitative reader, in a register a finance practitioner can read in one sitting.
...
- The probabilistic variant currently meets the joint criterion across the entire broad test universe (100 % drawdown-reduction coverage) and on the SPY single-ticker headline (Sharpe and terminal value above buy-and-hold). The baseline meets neither half.
...
- Reproducible scripts:
  - `experiments/runners/run_baseline.py` (`--tickers broad_universe --tag broad` for the broad-universe run)
...
- Documents:
  - `reports/generated/exports/Main_Dissertation_Draft.docx` (academic dissertation with Section 5.5 broad-universe table + Appendix B full per-ticker table)
