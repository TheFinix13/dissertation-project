...
\begin{itemize}
  \item \textbf{Risk-aware rewards.} Replacing $r_t = \Delta W_t$ with a
        ...
        buy-and-hold optimal. The preliminary run of
        Section~\ref{sec:res_riskaware} already shows the direction: at
        $\lambda = 0.25$, Deep Q's drawdown falls four-fold (6.0\% to
        1.4\%) with Sharpe rising to 0.68, while REINFORCE flips to the
        all-cash corner. The full study is a $\lambda$ sweep tracing the
        return-risk frontier each algorithm can reach, with multiple
        seeds.
