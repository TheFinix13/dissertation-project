...
familiar kinds of machine learning, then formalises the Markov Decision
Process, the mathematical object every later chapter manipulates. It then
...
\emph{agent}) interacts with a world (the \emph{environment}) in a loop:
it observes the situation, chooses an action, and receives back two
things only, a new situation and a numerical \emph{reward}. Nobody
...
predictions rather than waiting for final outcomes. The idea reaches its
...
outgrows memory. Either way, experience at one location teaches
...
transparently. Our Phase-0 results reproduce those weaknesses on cue
...
by gradient methods. Presciently for this project, they argue that the
\emph{objective} must be risk-adjusted (they propose a differential
...
\emph{negative} average Sharpe on their testbench. In their own words,
TDQN ``only barely surpasses the buy and hold strategy on these
particular bullish markets.'' Most tellingly, on the S\&P~500 ETF, the
very asset studied here, their reported test Sharpe for TDQN is 0.834,
\emph{identical} to buy-and-hold's 0.834. They observe that TDQN's
...
profit-seeking objectives. Their framing treats that result as a
footnote. This dissertation places it at the centre, explains it
mechanistically, and reproduces it with fully derived implementations.
...
buy-and-hold from public price history alone should not be expected. The
risk--return trade-off \cite{markowitz1952} implies that reducing risk,
not only raising return, is a legitimate form of success, measured here
by the Sharpe ratio \cite{sharpe1994} and maximum drawdown. This
dissertation therefore treats ``the agent rediscovers buy-and-hold'' as a
scientifically meaningful finding rather than a failure, and treats any
easy-looking victory as a signal to audit the experiment, not to
celebrate.
...
generates the observations. This is a natural reading of financial data,
and one raised explicitly in this project's supervision as the honest way
to think about what a price-only state misses.
...
policies) or take the route taken here: \emph{engineer the observation
...
detectable}. That is the gap this dissertation occupies. It offers a
single formulation (Chapter~\ref{ch:methodology}) and from-scratch
implementations whose every line is traceable to a derived equation
(Chapter~\ref{ch:implementation}, Appendix~\ref{app:trace}). The learners
are validated on known-outcome games first, then evaluated against
properly capitalised baselines under chronological splits, fee-inclusive
rewards and multiple seeds (Chapter~\ref{ch:results}).
