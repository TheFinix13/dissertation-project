...
The agent is paired with a forecaster that emits both a prediction of tomorrow's return and a confidence interval around that prediction. The width of that confidence interval is what the dissertation calls the \emph{uncertainty score} (Section~\ref{sec:uncertainty_score}), and is what is fed back into the agent's decision-making (Section~\ref{sec:env_and_contribution}).
...
The trading task is cast as a Markov decision process (MDP) with state space $\mathcal{S}$, action space $\mathcal{A}$, transition kernel $\mathcal{P}$, reward function $\mathcal{R}$, and discount factor $\gamma \in (0, 1]$. An MDP is the standard mathematical frame for sequential decision problems under uncertainty: at each time step the agent observes a state, chooses an action, receives a reward, and the environment transitions to a new state according to $\mathcal{P}$.
...
SPY (an ETF that tracks the S\&P 500 index) is presented in Sections~\ref{sec:headline_aggregate} and~\ref{sec:equity_curves} as a representative single-ticker case study before Section~\ref{sec:multi_ticker_robustness} expands the analysis to the full universe.

The data are split chronologically into three non-overlapping windows: a training window (1 January 2009 to 31 December 2018), a validation window (1 January 2019 to 31 December 2021), and a test window (1 January 2022 to 31 December 2025). The split is strict --- there is no random shuffling --- so that no information from the test window leaks into model selection.
...
\section{Uncertainty score}
\label{sec:uncertainty_score}

The uncertainty score is the dissertation's link between the forecaster and the policy. Two modes are evaluated: the headline aleatoric mode (the only mode in the original project proposal) and an epistemic mode added in response to project-proposal feedback.
...
\end{equation}
This score captures the data-side uncertainty Kendall and Gal call aleatoric~\parencite{kendall2017uncertainties}: it answers the question ``how chaotic does the market look right now to a forecaster trained on the past?''.
...
The combined epistemic + aleatoric uncertainty is the root-sum-of-squares of the spread of predicted means (epistemic) and the mean of predicted standard deviations (aleatoric):
\begin{equation}
  \label{eq:epistemic_uncertainty}
  \tilde{\sigma}_t = \sqrt{
    \underbrace{\mathrm{Var}_k\!\left[ \hat{\mu}_t^{(k)} \right]}_{\text{epistemic}}
    + \underbrace{\mathbb{E}_k\!\left[ \hat{\sigma}_t^{(k)} \right]^2}_{\text{aleatoric}}
  }.
\end{equation}
This combined quantity is then min--max normalised exactly as in Equation~\ref{eq:uncertainty_score} to produce the policy-facing uncertainty score. The epistemic mode is an experimental arm; the empirical comparison against the aleatoric mode is in Section~\ref{sec:epistemic_ablation}.
...
\textbf{Equation~\ref{eq:probabilistic_trade_size} is the mathematical contribution of this dissertation.} Everything else in the environment --- the state of Equation~\ref{eq:state}, the reward of Equation~\ref{eq:reward}, the constrained objective of Equation~\ref{eq:constrained_objective} --- is shared between the baseline and the probabilistic agent.
...
The protocol fixes the train, validation and test splits (Section~\ref{sec:data_preprocessing}). Three random seeds $\{7, 19, 42\}$ are used for both the baseline and the probabilistic agent in the Phase-1 budget; ten seeds are used in the extended-budget seed-stability check of Section~\ref{sec:extended_seed_stability}.
