...
This chapter reports the empirical results in nine sections. Section~\ref{sec:experimental_setup} restates the setup so the chapter can be read standalone. Section~\ref{sec:forecast_uncertainty} shows the forecaster's uncertainty signal in both modes, including the calibration check at Section~\ref{sec:forecaster_calibration}. Section~\ref{sec:headline_aggregate} is the single-ticker case study on SPY. Section~\ref{sec:equity_curves} shows the equity curves and the drawdown story. Section~\ref{sec:phase2_full_grid} presents the full Phase-2 results across 70 stocks. Section~\ref{sec:phase2_inference} is the paired statistical-inference pass on those results. Section~\ref{sec:ablation} reports the two-piece ablation of Equation~\ref{eq:probabilistic_trade_size} on a SPY pilot. Section~\ref{sec:walk_forward} reports the walk-forward validation. Section~\ref{sec:phase2_interpretation} is the honest interpretation of all Phase-2 evidence.
