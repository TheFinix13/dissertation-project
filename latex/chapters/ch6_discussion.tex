...
The corollary is that the baseline's small drawdown must not be mistaken for a risk-management win, and the fair drawdown benchmark is passive buy-and-hold, which actually carries market risk. On that benchmark the uncertainty guard is genuinely safer at the median: across the test grid its median maximum drawdown is about $22\,\%$ (aleatoric) against buy-and-hold's $\approx 26\,\%$ and the trailing stops' $26$--$33\,\%$ (Section~\ref{sec:drawdown_fair_comparison}, Figure~\ref{fig:drawdown_distribution}), and it achieves this while compounding to a median \$1.6\,M rather than sitting in cash. The honest qualifier is the tail: on the most volatile single names the guard's deepest drawdown ($\approx 57\,\%$) is worse than buy-and-hold's ($\approx 35\,\%$), so the drawdown advantage is a median-and-benchmark statement, not a per-ticker guarantee.
...
The Phase-2 grid (Section~\ref{sec:phase2_full_grid}) shows the uncertainty-aware agent finishing above its \$1\,M starting capital on roughly nine cells in ten. The per-ticker case-study basket in Appendix~\ref{app:per_ticker_phase1}, measured against passive buy-and-hold rather than against the baseline PPO, is the more demanding test: there the agent reduces drawdown relative to buy-and-hold on the typical ticker (its median maximum drawdown of about $22\,\%$ sits below buy-and-hold's $\approx 26\,\%$; Section~\ref{sec:drawdown_fair_comparison} and Figure~\ref{fig:drawdown_distribution}) but wins on terminal value only on a subset, and on the most volatile single names its worst-case drawdown can exceed buy-and-hold's. This section diagnoses where and why, because beating a flat baseline is easier than beating a rising market.
